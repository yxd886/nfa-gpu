	...
	if(counter!=PROCESS_TIME){
	  ...
	  if(coordinator_actor_->service_chain_.empty()==false){
		  ...
		  while(pos<size){
			  ...
			  int times=0;
			  coordinator_actor_->flow_size[pos]=it_actor->get_queue_ptr()->size();
		  ...
		  memcpy(coordinator_actor_->d_pkts,coordinator_actor_->pkts,PROCESS_TIME*bess::PacketBatch::kMaxBurst*sizeof(Pkt));

		  GPU_thread(coordinator_actor_,coordinator_actor_->d_pkts,coordinator_actor_->d_fs,pos,coordinator_actor_->flow_size,coordinator_actor_->flow_pos);
			 //gpu_thread.join();
	  }
