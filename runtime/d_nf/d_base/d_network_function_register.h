#ifndef D_NETWORK_FUNCTION_REGISTER_H
#define D_NETWORK_FUNCTION_REGISTER_H
...
class d_network_function_register{
public:
  ...
  bool register_nf(std::string nf_name, uint8_t nf_id, NFArgs&&... args){
    ...
    std::unique_ptr<d_network_function_base> ptr(new d_network_function_derived<NF, NFS>(nf_id,
  ...
  std::vector<d_network_function_base*> get_service_chain(uint64_t service_chain_type_sig);
...
private:
  ...
  std::unordered_map<uint8_t, std::unique_ptr<d_network_function_base>> id_nf_map_;
};

class d_static_nf_register{
public:
  static d_network_function_register& get_register();
};
